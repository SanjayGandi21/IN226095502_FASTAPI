...
def put_discount(category: str = Query(..., description='Category to discount'),discount_percent: int = Query(..., ge=1, le=99, description='% off')):
    updated = [] 
    for p in products: 
        if p['category'] == category: 
            p['price'] = int(p['price'] * (1 - discount_percent / 100)) 
            updated.append(p) 
        if not updated: return {'message': f'No products found in category: {category}'} 
...
def get_audit():
    in_=[p for p in products if p['in_stock']]
    out=[p['name'] for p in products if not p['in_stock']]
    tot=0
    for p in products:
        tot+=p['price']
    exp=max(products,key=lambda p:p['price'])

    return {'Total_Products':len(products),'in_stock_count':len(in_),'Out_of_stock_names':out,'Total_stock_value':tot,'most_expensive':{'Name':exp['name'],'Price':exp['price']}}
